...
static uint8_t game_death_screen;

uint8_t pixel_colours[2] = {0x3C, 0x13};

static void check_lives(int8_t asteroidNumber);
///////////////////////////////////////////////////////////
...
int8_t move_base(int8_t direction) {	
	...
	if (direction == MOVE_LEFT) {
		if (basePosition >= LEFT_MOVE_LIMIT) {
			if (asteroid_at(basePosition - 1, FIELD_BASE) != -1) {
				check_lives(asteroid_at(basePosition - 1, FIELD_BASE));
			} else {
				basePosition--;	
			}
		}
	} else {
		if (basePosition <= RIGHT_MOVE_LIMIT) {
			if (asteroid_at(basePosition + 1, FIELD_BASE) != -1) {
				check_lives(asteroid_at(basePosition + 1, FIELD_BASE));
			} else {
				basePosition++;	
...
int8_t fire_projectile(void) {
	...
	/* Code modified:
		The project will now spawn inside the turret, this is to handle the case where
		the asteroid is directly above the base station so the collision is detected.
		Previously the base station would still be hit
	*/
	if(numProjectiles < MAX_PROJECTILES && projectile_at(basePosition, 1) == -1) {
		// Have space to add projectile - add it at the x position of
		// the base, in row 2(y=2)
		newProjectileNumber = numProjectiles++;
		projectiles[newProjectileNumber] = GAME_POSITION(basePosition, 1);
		//redraw_projectile(newProjectileNumber, COLOUR_BASE);
		return 1;
	} else {
		return 0;
	}
...
/******** STUDENT FUNCTIONS, Author: Stephen Goodhill ********/

// Call this function every tick to create any asteroids that are
// waiting to be created
void create_missing_asteroids(void) {
	uint8_t i,x,y;
	
	// Make sure that there should actually be asteroids created
	if (numAsteroids != MAX_ASTEROIDS - 1) {
		for (i = numAsteroids; i < MAX_ASTEROIDS; i++) {
			do {
				x = (uint8_t)(random() % FIELD_WIDTH);
	...
	// Redraw all of the asteroids, the fixes the bug where the asteroids
	// would only render on y14, not y15.
	redraw_all_asteroids();
}

// Call this function every tick, this checks collisions between
// projectiles and asteroids. This is a better implementation because the check is more
// reliable, since it is called more frequently.
void check_projectile_collisions(void) {
	uint8_t i,x,y;
	for (i = 0; i < numProjectiles; i++) {
		x = GET_X_POSITION(projectiles[i]);
		y = GET_Y_POSITION(projectiles[i]);
...
// Call this function every tick, this checks collisions between asteroids
// and projectiles. This is a better implementation because the check is more
// reliable, since it is called more frequently.
...
// Checks if the base station is colliding with any ateroids
void check_base_collisions(void) {
	uint8_t x,y;
	x = basePosition;
	y = FIELD_BASE;
	if (asteroid_at(x,y) != -1) {
		check_lives(asteroid_at(x,y));
	} else if (asteroid_at(x+1,y) != -1) {
		check_lives(asteroid_at(x+1,y));
	} else if (asteroid_at(x-1,y) != -1) {
		check_lives(asteroid_at(x-1,y));
	} else if (asteroid_at(x,y+1) != -1) {
		check_lives(asteroid_at(x,y+1));
	}
}

// Sets the specified x,y coordinates of the led matrix to be orange
// This is called in game over to set the entire display to orange
void game_over_animation(uint8_t x, uint8_t y) {
	ledmatrix_update_pixel(x,y, pixel_colours[(uint8_t)(random() % 2)]);
}

// Returns 1 if the game is paused, returns 0 otherwise
int8_t is_game_paused(void) {
	return game_paused;
}

// Returns 1 if the player is dead, returns 0 otherwise
int8_t is_game_death_screen(void) {
	return game_death_screen;
}

// If the player is dead, restart the round
void update_death_screen(void) {
	if (is_game_death_screen()) {
		uint8_t x = 0, y = 0;
		while (x < 2) {
			while (y < 8) {
...
// Method to run the death animation for the players base station
...
// Moves the asteroids down 1 space
void advance_asteriods(void) {
	uint8_t x, y;
	int8_t asteriodNumber;
	asteriodNumber = 0;
	while (asteriodNumber < numAsteroids) {
		x = GET_X_POSITION(asteroids[asteriodNumber]);
		y = GET_Y_POSITION(asteroids[asteriodNumber]);
		if (y == FIELD_BASE) {
			remove_asteroid(asteriodNumber);
		} else {
			redraw_asteroid(asteriodNumber, COLOUR_BLACK);
			asteroids[asteriodNumber] = GAME_POSITION(x,y-1);
			redraw_asteroid(asteriodNumber, COLOUR_ASTEROID);
			asteriodNumber++;
...
// Decrements the number of lives remaining, if the number of lives is 0
// then the game is over
static void check_lives(int8_t asteroidNumber) {
	remove_asteroid(asteroidNumber);
	// Check if the player still has lives remaining
	if (get_remaining_lives() > 1) {
		// Redraw the base to black (this is the start of the death animation)
		uint8_t x = 0, y = 0;
		while (x < 2) {
			while (y < 8) {
				ledmatrix_update_pixel(x,y, COLOUR_ORANGE);
		...
		// This is run if the player has 0 lives left
		redraw_base(COLOUR_RED);
		decrement_lives(1);
		update_terminal_attributes(get_score(), get_remaining_lives());
		game_over = 1;
